                                    ...
                                    <table class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            foreach ($query->result() as $data) {
                                                $cek = $this->db->get_where('tb_nilai_pengetahuan', array('nisn' => $data->nisn, 'kodejdwl' => $detail->kodejdwl));
                                            ?>
                                                <form class="form-horizontal" action="<?= base_url('nilai_raport/simpanpengetahuan') ?>" method="post" enctype="multipart/form-data">
                                                    <tr>
                                                        <td><?= $no++ ?></td>
                                                ...
                                                if ($cek->num_rows() > 0) {
                                                    foreach ($cek->result() as $d) {
                                                        $ratarata = average(array($d->nilai1, $d->nilai2, $d->nilai3, $d->nilai4, $d->nilai5));
                                                        ...
                                                        $maxn = $this->db->query("SELECT ((nilai1+nilai2+nilai3+nilai4+nilai5)/5) as rata_rata, deskripsi FROM tb_nilai_pengetahuan where kodejdwl='$detail->kodejdwl' AND nisn='$d->nisn' ORDER BY rata_rata DESC LIMIT 1;")->row();
                                                        $rapn = $this->db->query("SELECT sum((nilai1+nilai2+nilai3+nilai4+nilai5)/5)/count($d->nisn) as raport FROM tb_nilai_pengetahuan where kodejdwl='$detail->kodejdwl' AND nisn='$d->nisn'")->row();
                                                ...
                                                ?>
                                                        <tr>
                                                            ...
                                                            <td><?= $d->deskripsi ?></td>
                                                            <td align="center">
                                                                <a href="<?= base_url('nilai_raport/hapuspengetahuan/'.$d->id_nilai_pengetahuan.'/'.$detail->kodejdwl) ?>" class="btn btn-xs btn-danger" onclick="return confirm('Apa anda yakin untuk hapus Data ini?')"><span class="glyphicon glyphicon-remove"></span></a>
                                                            </td>
                                                        </tr>
                                                    <?php } ?>
                                                    <tr>

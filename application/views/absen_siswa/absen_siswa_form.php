                    ...
                    <div class="box-body">
                        ...
                        <form action="<?= base_url('absen_siswa/lihat_absen') ?>" method="POST" class="col-md-7 pull-right" style="margin-bottom:5px">
                            <div class="col-xs-3"><select name="tgl" class="form-control">
                                    ...
                                    <?php
                                    for ($i = 1; $i <= 31; $i++) { ?>
                                        <option value="<?= $i ?>" <?= $i == date('d') ? 'selected' : null ?>><?= $i ?></option>
                                    <?php } ?>
                                </select></div>
                            <div class="col-xs-4"><select name="bln" class="form-control">
                                    <option selected="">- Bulan -</option>
                                    <option value="1" <?= date('m') == '1' ? 'selected' : null ?>>Januari</option>
                                    <option value="2" <?= date('m') == '2' ? 'selected' : null ?>>Februari</option>
                                    <option value="3" <?= date('m') == '3' ? 'selected' : null ?>>Maret</option>
                                    <option value="4" <?= date('m') == '4' ? 'selected' : null ?>>April</option>
                                    <option value="5" <?= date('m') == '5' ? 'selected' : null ?>>Mei</option>
                                    <option value="6" <?= date('m') == '6' ? 'selected' : null ?>>Juni</option>
                                    <option value="7" <?= date('m') == '7' ? 'selected' : null ?>>Juli</option>
                                    <option value="8" <?= date('m') == '8' ? 'selected' : null ?>>Agustus</option>
                                    <option value="9" <?= date('m') == '9' ? 'selected' : null ?>>September</option>
